...
export default function LessonDiscussionsPage({
  ...
  return (
    <div className="min-h-screen pt-24 bg-gradient-to-b from-background to-muted p-6 dark:from-background dark:to-muted">
        ...
        <div className="space-y-4">
          {discussions.map((discussion) => (
            <motion.div
              ...
              key={discussion.id}
            >
              <Card className="hover:shadow-xl transition-all duration-300 border-muted-foreground/20 dark:bg-background/95 bg-white">
                <CardContent className="pt-6">
                  <div className="flex items-start space-x-4">
                    <div className="h-10 w-10 rounded-full bg-primary/10 flex-shrink-0 relative shadow-sm dark:bg-primary/20">
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-xs text-muted-foreground bg-muted/50 dark:bg-muted/30 px-2 py-1 rounded-full">
                      ...
                      <p className="text-sm text-foreground/80 bg-muted/30 dark:bg-muted/20 p-3 rounded-lg">
                        {discussion.content}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 pl-14 space-y-4">
                    {discussion.replies.map((reply) => (
                      ...
                      >
                        <div className="h-8 w-8 rounded-full bg-primary/10 flex-shrink-0 relative shadow-sm dark:bg-primary/20">
                        ...
                        <div className="flex-1 bg-muted/20 dark:bg-muted/10 p-3 rounded-lg group-hover:bg-muted/30 dark:group-hover:bg-muted/20 transition-colors">
                    ...
                    <div className="flex gap-3 mt-4">
                      <Textarea
                        ...
                        className="min-h-[100px] flex-1 bg-background resize-none focus:ring-2 ring-primary/20 dark:bg-muted/10"
                      />
                      <Button
                        onClick={() => handleReply(discussion.id)}
                        disabled={!replyInputs[discussion.id]?.trim()}
                        className="self-start bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg hover:shadow-xl transition-all disabled:opacity-50"
                      >
                        Reply
                      </Button>
                    </div>
          ...
          {discussions.length === 0 && (
            <Card className="border-dashed border-2 border-muted-foreground/20 dark:bg-background/95 bg-white">
